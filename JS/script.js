...
function addSection(type) {
    const section = {
        type,
        content: type === 'orderedList' || type === 'unorderedList' || type === 'checkbox' || type === 'blockQuote' || type === 'codeBlock' ? [] : "",
...
function renderSections() {
    ...
    sections.forEach((section, index) => {
        ...
        sectionDiv.innerHTML = `
            <label class="section-label">${getSectionLabel(section.type)}</label>
    ...
    const headers = sections
        .map((section, index) => {
            if (section.type === "header2") {
                const anchorName = formatAnchorName(section.content || "untitled-header");
                return `<a href="#${anchorName}">${section.content || "Untitled Header"}</a>`;
            }
            return null;
        })
...
function getSectionLabel(type) {
    switch (type) {
        case "header1": return "New Section";
        case "header2": return "Header ##";
        case "header3": return "Header ###";
        case "header4": return "Header ####";
        case "header5": return "Header #####";
        case "header6": return "Header ######";
        case "text": return "Text Block";
        case "unorderedList": return "Unordered List";
        case "orderedList": return "Ordered List";
        case "checkbox": return "Checkbox Section";
...
function validateLanguage(index, language) {
    const supportedLanguages = [
        "javascript", "python", "java", "c", "c++", "c#", "ruby", "go",
        "php", "typescript", "swift", "html", "css", "kotlin", "r", "bash",
        "shell", "sql", "perl", "rust", "dart", "scala", "lua", "powershell",
        "json", "yaml", "xml", "matlab", "groovy", "pascal", "haskell", 
        "fortran", "objective-c", "visualbasic", "vb.net", "abap", "scheme",
        "elixir", "f#", "assembly", "clojure", "cobol", "d", "erlang", 
        "nim", "ocaml", "pl/sql", "sas", "ada", "awk", "julia", "prolog", 
        "racket", "smalltalk", "tcl", "verilog", "vhdl", "vbscript"
    ];
    ...
    try {
        ...
        const contentMarkdown = sections
            .map((section) => {
                const anchorName = formatAnchorName(section.content || "untitled-header");
                switch (section.type) {
                    case "header1": return `# ${section.content}`;
                    case "header2": return `## ${section.content}`;
                    case "header3": return `### ${section.content}`;
                    case "header4": return `#### ${section.content}`;
                    case "header5": return `##### ${section.content}`;
                    case "header6": return `###### ${section.content}`;
                    case "text": return section.content || "";
                    case "unorderedList": return (section.content || []).map((item) => `- ${item}`).join("\n");
                    case "orderedList": return (section.content || []).map((item, i) => `${i + 1}. ${item}`).join("\n");
                    case "checkbox": return (section.content || []).map((item) => `- [ ] ${item}`).join("\n");
                    case "blockQuote": return (section.content || []).map((item) => `> ${item}`).join("\n");
                    case "codeBlock": {
                        const codeLanguage = section.language ? `\`\`\`${section.language}` : "```";
                        const codeContent = Array.isArray(section.content) ? section.content.join("\n") : section.content || "";
                        return `${codeLanguage}\n${codeContent}\n\`\`\``;
                    }
                    case "lineBreak": return `<br>`;
                    case "horizontalRule": return `---`;
                    case "image": return `![Image](${section.content})`;
                    case "url": return `[${section.content.text || "Link"}](${section.content.url || ""})`;
                    default: return "";
                }
            })
            .join("\n\n");
